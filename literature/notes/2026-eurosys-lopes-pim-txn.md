...
- [paper] 同期プリミティブは acquire / release のみ(256-bit atomic register 上で動作し、
  ...
  aliasing あり)。CAS も read-write lock も無い。浮動小数点はソフトウェアエミュレーション。
...
- [paper] エポック内の同一 DPU 上では、決定的(= distributed)subtransaction を先に実行し、
  ...
  conflict graph のエッジが G_L → G_D に向くことを保証すれば全体が acyclic、と述べる
  (§3.2, footnote 1, p.6)。
...
- [paper] **Execution — 混合実行戦略** (§3, §3.2): local トランザクションはグローバル順序の
  ...
  local 処理では非決定的 CC が決定的比で最大 1.5× 速い (§3.2, §4.3)。
...
- Headline [paper]:
  - TPC-C ミックス全体: 全構成で CPU ベースラインを上回り、DPU 数と共に差が拡大
    ...
    - [question] abstract/§1 は「up to 6.75×」、§4.2.1/§5 は「up to 10×」、さらに本文の
...
- [question] §3.2 footnote 1 の conflict graph エッジ向き(G_L → G_D)は「決定的を先に
