...
- [question] fast quorum の表記が §3.1・§3.3.2 では ⌈3n/4⌉+1、Alg.1 の解説
  (§3.3.1 の 7a)では ⌊3n/4⌋+1 と floor/ceiling が揺れている。N=3 だと
...
- [paper] **TxDAG と leading edge**: 各サーバはコミット済み + prepared の履歴を
  ...
  コミットできる (§2 [contribution], §3, Fig. 1, Fig. 2)。
...
- [paper] **coordinator 故障回復 (§3.6)**: 全サーバは自分の PREPARED 応答(LE メタ
  ...
  疑ったサーバは RECOVER_PREPARED を全サーバに送る(単独決定は不可、多数決 outcome が
...
- Inferred [inference]:
  ...
  - fast quorum の floor/ceiling 表記の不整合(§3.1/§3.3.2 vs §3.3.1)。N=3 での
