...
一貫性を持つ。評価では term-match 38.6→6043.9 QPS(対 scan+LIKE。P99
1197.4→4.3ms)、vector top-k 526.3→3752.4 QPS(対 TiFlash vector index)、
混合 31.1→1169.2 QPS(対 scan)、OLTP への影響は
...
- [paper] 現実の対処は 2 択: (a) 外部システム(検索エンジン / ベクタ DB)併設 —
  ...
  急速に進化する retrieval 技術の取り込みを遅らせる (§I, §II-A)。
...
- 2026-07-06: 検証パスによる修正(密結合の欠点のアンカーを §I, §II-A に訂正、TL;DR の vector/mixed ベースラインを scan と明確に区別)
