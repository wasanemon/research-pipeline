...
- [paper] 既存 HM 最適化が CXL で効かない理由 (§2.3):
  ...
  - PM 系(PACTree の fat unordered leaf + async SMO)は、CXL DRAM に PM の
    ...
    空きスロット探索、テール遅延 1.3〜1.9×)(Fig. 3b, 4b)。
...
- [paper] 対象は「値を leaf に持つ」木構造インデックス(B+tree、radix tree/ART、Masstree)
  (§1 footnote)。CXL 1.1 メモリ拡張カード、単一ソケット(例外: 大規模データセットの
  スケーラビリティ試験のみ 1TB dual-socket 機で downclocked remote NUMA により CXL を
  エミュレート)、32GiB DRAM + 32GiB CXL-DRAM、Xeon Platinum 8468V (§6.1, §6.4)。
...
- [paper] **構造対応の移動** (§4.2): promotion は leaf から祖先を再帰的に fast へ
  ...
  アルゴリズムで single-boundary とホットパス局所性を保つ(L_demote より下のレベルは対象外)
  (Alg. 1, Fig. 10)。
...
- [paper] 動的ワークロード(ホット領域を 60 秒ごとに移動): 2 秒で 84% 回復、最大 6 秒で
  ...
  80% で大半のワークロードにおいてほぼ全量 fast 相当 (Fig. 16b-c)。
- [paper] DB 統合 (§5, §6.7): in-memory OLTP DB の Silo に統合(Masstree インデックスを
  置換、SIDLE のバックグラウンドワーカー 5 スレッドを Silo の既存バックグラウンド
  モジュールに組込)。TPC-C でスループット S-ART +26.3% / S-Masstree +29.1%、テール遅延
  S-ART −31.4〜39.9% / S-Masstree −11.2〜36.7%(対ベースライン)(Fig. 22)。
- [inference] カバーされないもの: マルチソケット+CXL の複合階層(性能実験は原則
  単一ソケット)、CXL 2.0/3.0 スイッチ経由のより大きい遅延ギャップでの検証(言及は
  あるが実験なし)、バッファプールや MVCC バージョン管理と fast メモリを奪い合う
  DBMS での挙動(Silo 統合 (§6.7) はあるが、Silo はバッファプールを持たない
  in-memory OLTP DB)。
...
- 2026-07-06: 検証パスによる修正(demotion 対象外の境界を「L_demote より下」に訂正;
  PM 系のテール遅延 1.3〜1.9× のアンカーを Fig. 3b, 4b に訂正; 単一ソケットの例外
  (§6.4 大規模試験は dual-socket + NUMA エミュレーション)を追記; §5/§6.7 の
  Silo + TPC-C 統合結果を [paper] として追加し、「DBMS 統合はカバー外」としていた
  [inference] を修正; Fig. 16c の「大半のワークロード」限定を追記)
