...
- Inferred [inference]:
  ...
  - 利得の源泉は「disk ≫ RDMA」のギャップ。NVMe では利得が SATA 比で縮む:
    YCSB-C 2.6× (§5.8) → 2.2×、YCSB-A 5.4× (§5.8) → 2.5× (§5.4)。
    さらに速いストレージ(PM や CXL-SSD 級)や、より遅いネットワーク
...
- [inference] CBA (§3.4) は「単一の R をグローバルに決める」1変数モデル。buffer
  ...
  (§5.11 で構築済み。artifact 自体は公開されているが (Appendix A)、シミュレータの
  同梱は Appendix に明記されておらず要確認)を再現するかたちで安価に検証できる。
...
- 2026-07-06: 検証パスによる修正(NVMe 利得の縮小を同一ワークロード比較 YCSB-C 2.6×→2.2× / YCSB-A 5.4×→2.5× に訂正(旧記述は uniform microbench の 5.3× を YCSB-C と誤対応); シミュレータの artifact 同梱は Appendix 未記載のため「要確認」に弱め)
